...
namespace VoxelWorld.Renderers
{
	public class ChunkRenderer
	{
		...
		private readonly int[] tranglesBuffer;
		...
		public Mesh[] GetMesh()
		{
			if (this.meshInvalidated)
				GenerateMesh();
			return new[] { this.cutoutMeshBuffer.GetMesh(), this.transparentMeshBuffer.GetMesh() };
		}
				...
				for (int z = this.min[2]; z < this.max[2]; z++)
				{
					for (int x = this.min[0]; x < this.max[0]; x++)
					{
						...
						for (int i = 0; i < BlockService.FaceOrder.Length; i++)
						{
							...
							activeMeshBuffer.GetVertexCount(out int vertexCount);
							for (int j = 0; j < BlockService.FaceTriangleOrder.Length; j++)
							{
								this.tranglesBuffer[j] = vertexCount - 1 - BlockService.FaceTriangleOrder[j];
							}
							activeMeshBuffer.AddTriangles(this.tranglesBuffer);
